...
app.get("/", (req, res) => {
  ...
  const branch = imagedb.prepare(`SELECT * FROM branchdb`).all();

  // console.log(images);
  res.render("home", { images , branch });
...
app.get("/9171441509", (req, res) => {
  ...
  res.render("admin", { images , branch });
});
...
app.post("/image", upload.array("images", 5), async (req, res) => {
  ...
  for (const imageBuffer of images) {
    try {
      imagedb
        .prepare("INSERT INTO imagedb (name,image) VALUES (?,?)")
        .run(tab, imageBuffer);
    } catch (error) {
      console.log(error);
    }
...
app.post("/addbranch", (req, res) => {
  ...
  imagedb
  .prepare("INSERT INTO branchdb (name,url) VALUES (?,?)")
  .run(name, url);
  res.redirect('/9171441509')
});

app.get("/deletebranch/:id", (req, res) => {
  const id = req.params.id;


  imagedb.prepare(`DELETE FROM branchdb WHERE id=?`).run(id);
